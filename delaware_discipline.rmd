...
# Run the below command if you need to clear your workspace
rm(list=ls())
gc()
```
...
Make the race categories easier to type.
```{r}
...
# This function will make sure the labels are all correct.
...
shows the school year (2015, 2016, 2017, 2018, 2020, 2021, 2022, 2023), 
<district_code> is a column we will ignore, but it shows the number 
code of each district. The one we want is 0, the state of Delaware. 
<district> shows in words the name of each district. <school_code> 
shows the number code of each school. <organization> shows the school 
name <race> shows what race the statistics address <special_demo> is a 
more clear version of the <sub_group> column. It shows what subgroup 
the statistics contained in the row are for <sub_group> shows the above 
in a more complicated form -- i.e. instead of reading "ESL students", 
it might read "Hispanic/Latino/1st Grade/Out-of-School Suspension with 
CDAP/ESL students." This would be helpful, but it doesn't list all the 
statuses, e.g. what district one is looking at <category> displays what
type of discipline was applied - in-school discipline, out-of-school 
discipline, expulsion, etc. <rowstatus> shows whether or not a row has 
been ommitted out of privacy concerns - if the pool of students is small 
enough, it would be possible to identify which students were disciplined, 
in theory. <students> shows the number of STUDENTS who were disciplined 
with <category>. <enrollment> shows the numer of students who fit into a
...
District Code (this argument is mandatory) - Race (mandatory) - Grade (not mandatory; if no input, 
defaults to "all students") - Gender (not mandatory; if no input defaults to "all students") -
Discipline type (not mandatory: if no input, no argument is passed; i.e. in-school suspension, 
out-of-school suspension, expulsion) - Sub-group (not mandatory) - School year (not mandatory)

If interested in all students in a particular category, e.g. the average number of students of 
all races, write "all" (any capitalization), leave the argument out, or write "NULL".

The <process_race_data> function handles all these arguments.
```{r}
...
The above functions make sorting and filtering the data a lot easier - now one just has to plug in the dataframes and the district code. All other arguments are optional.
That's what the below does.
``` {r}
hisp_overall <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_hisp) 
print(hisp_overall)
native_am_overall <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_native_am)
print(native_am_overall)
black_overall <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_black)
print(black_overall)
white_overall <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_white)
print(white_overall)
asian_am_overall <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_asian_am)
print(asian_am_overall)
multi_racial_overall <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_multi_racial)
print(multi_racial_overall)
hawaiian_pi_overall <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_hawaiian_pi)
print(hawaiian_pi_overall)
```

A word of caution - make sure no values in asian_am_overall are 'NA'. The code
is correct, but R does not consistently replace "Asian" with
...
```{r}
overall <- list()
# This will combine all the results into a dataframe for graphing purposes
overall <- hisp_overall |> 
    full_join(native_am_overall) |> 
    full_join(black_overall) |> 
    full_join(white_overall) |> 
    full_join(asian_am_overall) |> 
    full_join(multi_racial_overall) |> 
    full_join(hawaiian_pi_overall) |> 
    mutate(
...
write_csv(overall, "delaware_discipline_statewide_overall.csv")
...
overall_graph <- overall_graph |> 
    ...
    labs(
        title="Multi-racial and Native American students are \nfar more likely to face discipline in Delaware.",
        x= "race",
        y="avg"
    ) +
    theme_economist() +
...
```{r}
hisp_in_school_sus <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code=0, race_name = name_hisp, disc_type=name_in_school_sus)
print(hisp_in_school_sus)
native_am_in_school_sus <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code=0, race_name = name_native_am, disc_type=name_in_school_sus)
print(native_am_in_school_sus)
black_in_school_sus <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code=0, race_name = name_black, disc_type=name_in_school_sus)
print(black_in_school_sus)
white_in_school_sus <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code=0, race_name = name_white, disc_type=name_in_school_sus)
print(white_in_school_sus)
asian_am_in_school_sus <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code=0, race_name = name_asian_am, disc_type=name_in_school_sus)
print(asian_am_in_school_sus)
multi_racial_in_school_sus <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code=0, race_name = name_multi_racial, disc_type=name_in_school_sus)
print(multi_racial_in_school_sus)
hawaiian_pi_in_school_sus <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code=0, race_name = name_hawaiian_pi, disc_type=name_in_school_sus)
print(hawaiian_pi_in_school_sus)
```

Making a graph for in-school suspensions.
``` {r}
...
write_csv(in_school_suspensions, "delaware_discipline_statewide_in_school_suspens.csv")
...
in_school_suspensions_graph <- in_school_suspensions_graph |> 
    ...
    geom_col(aes(x= race_ordered, y=per_capita), fill="darkblue") +
    labs(
        title="Native American and multi-racial students are far more likely \n to face in-school discipline in Delaware.",
        x= "race",
        y="avg"
    ) +
    theme_economist() +
...
Very interesting - Native Hawaiian/Pacific Islander students, followed by multi-racial students are most likely to face in-school discipline, considering the population. Now, let's see about out-of-school suspensions - does the trend continue?
```{r}
hisp_oos_cdap <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code=0, race_name=name_hisp, disc_type=name_oos_cdap)
print(hisp_oos_cdap)
native_am_oos_cdap <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code=0, race_name=name_native_am, disc_type=name_oos_cdap)
print(native_am_oos_cdap)
black_oos_cdap <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code=0, race_name=name_black, disc_type=name_oos_cdap)
print(black_oos_cdap)
white_oos_cdap <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code=0, race_name=name_white, disc_type=name_oos_cdap)
print(white_oos_cdap)
asian_am_oos_cdap <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code=0, race_name=name_asian_am, disc_type=name_oos_cdap) 
print(asian_am_incidents) 
multi_racial_oos_cdap <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code=0, race_name=name_multi_racial, disc_type=name_oos_cdap)
print(multi_racial_oos_cdap)
hawaiian_pi_oos_cdap <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code=0, race_name=name_hawaiian_pi, disc_type=name_oos_cdap)
print(hawaiian_pi_oos_cdap)
```

Making a graph for out-of-school suspensions with CDAP.
``` {r}
...
out_of_school_cdap_suspensions_graph <- out_of_school_cdap_suspensions_graph |> 
    ...
    labs(
        title="Asian students are by far the most likely to face \nout-of-school discipline with CDAP in Delaware.",
        x= "race",
        y="avg"
    ) +
    theme_economist() +
...
```{r}
hisp_oos_wo_cdap <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_hisp, disc_type = name_oos_wo_cdap)
native_am_oos_wo_cdap <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_native_am, disc_type = name_oos_wo_cdap)
black_oos_wo_cdap <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_black, disc_type = name_oos_wo_cdap)
white_oos_wo_cdap <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_white, disc_type = name_oos_wo_cdap)
asian_am_oos_wo_cdap <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_asian_am, disc_type = name_oos_wo_cdap)
multi_racial_oos_wo_cdap <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_multi_racial, disc_type = name_oos_wo_cdap)
hawaiian_pi_oos_wo_cdap <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_hawaiian_pi, disc_type = name_oos_wo_cdap)
```

Making a graph for out-of-school suspensions without CDAP.
``` {r}
...
write_csv(out_of_school_wo_cdap_suspensions, "delaware_discipline_statewide_out_of_school_suspensions_WITHOUT_CDAP.csv")
...
```{r}
hisp_oos <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_hisp, disc_type = name_oos)
native_am_oos <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_native_am, disc_type = name_oos)
black_oos <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_black, disc_type = name_oos)
white_oos <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_white, disc_type = name_oos)
asian_am_oos <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_asian_am, disc_type = name_oos)
multi_racial_oos <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_multi_racial, disc_type = name_oos)
hawaiian_pi_oos <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_hawaiian_pi, disc_type = name_oos)
```

Making a graph for out-of-school suspensions
``` {r}
...
write_csv(oos, "delaware_discipline_statewide_out_of_school_suspensions.csv")
...
oos_graph <- oos_graph |> 
    ...
    geom_col(aes(x= race_ordered, y=avg_per_capita_disciplines), fill="darkblue") +
    labs(
        title="Black and multi-racial students are most likely to face out-of-school \nsuspensions in Delaware.",
        x= "race",
        y="avg"
    ) +
    theme_economist() +
...
print(oos_graph)
ggsave("renderings/delaware_out_of_school_suspensions.png", oos_graph)
```
...
```{r}
hisp_expulsion <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_hisp, disc_type = name_expulsion)
native_am_expulsion <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_native_am, disc_type = name_expulsion)
black_expulsion <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_black, disc_type = name_expulsion)
white_expulsion <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_white, disc_type = name_expulsion)
asian_am_expulsion <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_asian_am, disc_type = name_expulsion)
multi_racial_expulsion <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_multi_racial, disc_type = name_expulsion)
hawaiian_pi_expulsion <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_hawaiian_pi, disc_type = name_expulsion)
```

Making a graph for expulsion data.
``` {r}
...
write_csv(expulsions, "expulsions.csv")
...
expulsions_graph <- expulsions_graph |> 
    ...
    geom_col(aes(x= race_ordered, y=avg_per_capita_disciplines), fill="darkblue") +
    labs(
        title="Black and white students are most likely to face expulsions in Delaware.",
        x= "race",
        y="avg"
    ) +
    theme_economist() +
...
```
 ...
 students are consistently the most likely to face discipline. Are
...
median_overall_suspension_length <- length_avg |> 
    ...
    geom_col(fill="blue") +
    labs(
        title="Native American students, followed by white students, get the longest \n punishments per capita",
        x= "Race",
...
# Make sure these are installed! If not, use the install.packages("") function
library(doParallel)
library(foreach)
...
results_list <- foreach(district = district_codes, .combine = bind_rows) %dopar% {
    ...
    for (category in disc_type) {
        for (race in race_names) {
            ...
            result <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = district, race_name = race,
                                        disc_type = category)

            # Check if per_capita is NULL and replace with NA
            per_capita <- ifelse(is.null(result$per_capita), NA, result$per_capita)
