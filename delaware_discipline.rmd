...
Discipline type (not mandatory: if no input, no argument is passed; i.e. in-school suspension, out-of-school suspension, expulsion) - Sub-group (not mandatory)
...
native_am_incidents <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_native_am)
print(native_am_incidents)
black_incidents <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_black)
print(black_incidents)
white_incidents <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_white)
print(white_incidents)
asian_am_incidents <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_asian_am)
print(asian_am_incidents)
multi_racial_incidents <- process_race_data(pop_df=delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_multi_racial)
print(multi_racial_incidents)
hawaiian_pi_incidents <- process_race_data(pop_df = delaware_pop, discipline_df = delaware_discipline, dis_code = 0, race_name = name_hawaiian_pi)
print(hawaiian_pi_incidents)
```

A word of caution - make sure no values in asian_am_incidents are 'NA'. The code
